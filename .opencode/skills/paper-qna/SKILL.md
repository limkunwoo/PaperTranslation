...
**장면 아키텍처 유형:**
- **공유 씬** (Q4, Q5, Q8): `buildInteractiveScene(canvasId, sliderId, valId, opts)` 래퍼 — 옵션으로 라벨/법선/그래디언트 토글
- **독립 씬** (Q11): 고유한 UI 컨트롤(반사 토글 버튼 등)이 필요할 때 별도 함수 작성. `buildThreejsHtml()`에서 추가 UI 요소 생성 분기 포함
